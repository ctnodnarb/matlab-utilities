function printRasterPdf(fileName, figureSize, units, dpi, extraSpace, figHandle)
	...
	%   dpi - Optional parameter.  The resolution of the output in dots per
	%		inch.  If omitted, 300 will be used.  The default of 300 will 
	%		also be used if [] or 0 is passed in.
	...
	if(nargin < 4 || isempty(dpi) || dpi == 0)
		dpi = 300;
	end
	if(nargin < 5 || isempty(extraSpace))
		extraSpace = [0 0 0 0];
	end	
	if(nargin < 6)
		figHandle = gcf;
	...
	% Output the figure
	print(figHandle, '-dpdf', fileName, ['-r' int2str(dpi)]);
